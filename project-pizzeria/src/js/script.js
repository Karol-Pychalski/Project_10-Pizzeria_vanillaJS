...

{
  ...
  class Product{
    ...
    initAccordion(){
      ...
      const accordionTrigger = document.select.menuProduct.clickable('.product__header');   //?

      /* START: add event listener to clickable trigger on event click */
      thisProduct.accordionTrigger.addEventListener('click', function(event) {  //było: clickableTrigger.addEventListener('click', function(event){ -> zmiana w 7.6
    ...
    processOrder(){
      ...
      for(let paramId in thisProduct.data.params){
        ...
        for(let optionId in param.options){
          ...
          if(formData[paramId] && formData[paramId].inculdes(optionId)){
